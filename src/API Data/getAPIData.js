...
import { CITY_CODE_URL, CUISINES_LIST_URL, ENTITY_URL, RESTAURANTS_LIST_URL } from "../URLs/URL";
...
export let getRestaurants = (entityId, entityType, cuisineIds, start, end) => {
    let cuisines = encodeURIComponent(cuisineIds);
    return Axios.get(
        RESTAURANTS_LIST_URL.replace('%s1', entityId).replace('%s2', entityType).replace('%s3', cuisines)
